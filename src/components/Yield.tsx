...
const Yield: React.FC = () => {
  ...
  const [accruedYield, setAccruedYield] = useState<string>('--');
    ...
    async function fetchYield() {
      if (provider && account) {
        try {
          ...
          setStakedAmount(yieldRaw.toString());
          // For demo, accruedYield = stakedAmount (replace with real calculation)
          setAccruedYield(yieldRaw.toString());
        } catch (err) {
          setStakedAmount('--');
          setAccruedYield('--');
        }
  ...
  const handleStartEarning = async () => {
    ...
    try {
      ...
      const tx = await supplyToAave(provider!, signer, ethers.parseUnits('1', 18));
      await tx.wait();
    ...
    <div className="p-6 max-w-2xl mx-auto">
      ...
      <div className="mb-8 p-4 bg-white rounded shadow">
        ...
        <button
          ...
          disabled={isSupplying || !signer}
        >
          {isSupplying ? 'Supplying...' : 'Start Earning'}
        </button>
      </div>
      <div className="mb-8 p-4 bg-white rounded shadow">
        <h2 className="text-xl font-semibold mb-2">Your Yield Stats</h2>
        <ul className="text-gray-800">
          <li>Staked Amount: <span className="font-mono">{stakedAmount}</span></li>
          <li>APY: <span className="font-mono">--%</span></li>
          <li>Accrued Yield: <span className="font-mono">{accruedYield}</span></li>
        </ul>
      </div>
